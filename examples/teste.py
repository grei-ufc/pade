...
class Time(TimedBehaviour):
    ...
    def __init__(self, agent, notify):
        super(Time, self).__init__(agent, 2)
        self.notify = notify
        self.inc = 0
        self.agent = agent
    ...
    def on_time(self):
        ...
        body = {
            ...
            'sender': self.agent.aid.name
        }
        message.set_content(str(json.dumps(body)))
        self.notify(message)
        self.inc += 0.1
...
if __name__ == '__main__':
    ...
    for i in range(agents_per_process):        
        ...
        agent_name = 'agent_subscriber_1:{}'.format(port)
        agent_sub_1 = AgentSubscriber(AID(name=agent_name), msg)
        agents.append(agent_sub_1)
        port += 1

        agent_name = 'agent_subscriber_2:{}'.format(port)
        ...
        agent_name = 'agent_subscriber_3:{}'.format(port)
        agent_sub_3 = AgentSubscriber(AID(name=agent_name), msg)
